...
*   **Bundled gcloud needs CLOUDSDK_PYTHON (P3-GCP):** the pinned SDK under `deploy\bin\google-cloud-sdk` requires a Python interpreter. Run `deploy\07_configure_gcloud.ps1` once after install — it sets the **Machine-scope** `CLOUDSDK_PYTHON` to the project venv's python. Machine-scope variables are inherited by services **only after they restart**, so pass `-RestartServices` (or restart AamBackupAgent/AamWatchdog/AamPrefectServer manually; note these services ignore SCM STOP — use `taskkill /PID <svcPid> /T /F` then `Start-Service`). Without this variable every FY-rollover ARCHIVE transition silently no-ops.
*   **Disabled legs are never scheduled (P2-SCHED):** setting `lan.enabled: false` or `cloud.enabled: false` in config.yaml now means the matching deployment is **not registered** at boot AND any stale server-side deployment is **paused** (`launch._reconcile_disabled_legs`, pause-not-delete — reversible via resume when re-enabled). Toggling an enabled flag requires an agent restart for registration/reconciliation to run. A disabled leg therefore produces NO flow runs at all — silence in the history for a disabled leg is correct behavior.
